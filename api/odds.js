...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'odds') {
      ...
      // 預設用 Bet365（已驗證可用，水位低、市場完整）。
      // 前端可傳 bookmakers 參數覆蓋，例如 bookmakers=Bet365,Betfair+Exchange
      const bookmakers = req.query.bookmakers || 'Bet365';
      const cacheKey = `odds_${eventId}_${bookmakers}`;
      const ODDS_TTL = 45 * 1000; // 賠率快取 45 秒，比賽進行中仍相對新鮮
      if (cache[cacheKey] && (Date.now() - cache[cacheKey].ts) < ODDS_TTL) {
        res.setHeader('X-Cache', 'HIT');
        res.status(200).json(cache[cacheKey].data);
        return;
      }
      const url = `${ODDS_API_BASE}/odds?apiKey=${apiKey}&eventId=${encodeURIComponent(eventId)}&bookmakers=${encodeURIComponent(bookmakers)}`;
      const upstream = await fetch(url);
      if (!upstream.ok) {
        const errText = await upstream.text();
        res.status(upstream.status).json({
